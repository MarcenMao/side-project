...
$totalPages_productList = ceil($totalRows_productList/$maxRows_productList)-1;$maxRows_productList = 5;
$pageNum_productList = 0;
if (isset($_GET['pageNum_productList'])) {
  $pageNum_productList = $_GET['pageNum_productList'];
}
...
mysql_select_db($database_storeDB, $storeDB);
$query_productList = "SELECT * FROM product ORDER BY pno ASC";
$query_limit_productList = sprintf("%s LIMIT %d, %d", $query_productList, $startRow_productList, $maxRows_productList);
$productList = mysql_query($query_limit_productList, $storeDB) or die(mysql_error());
$row_productList = mysql_fetch_assoc($productList);
...
<h3><a href="addnewpd.php">上架產品</a>  <a href="<?php echo $logoutAction ?>">登出</a></h3></h1>
<p>
<p>既有產品: </p>
<table border="1" bgcolor="#66FFCC">
  <tr>
    ...
    <th width="181" scope="col">刪除</th>
    <th width="181" scope="col">更新</th>
  ...
  <tr>
    ...
    <td><?php echo $row_productList['description']; ?></td>
     <td  class="dellll"><a href="delpd.php?pno=<?php echo $row_productList['pno']; ?>">✖</a></td>
      <td><a href="updatapd.php?pno=<?php echo $row_productList['pno']; ?>"><?php echo $row_productList['pno']; ?></a></td>
  </tr>
